...
zvec-go 是 [zvec](https://github.com/alibaba/zvec) 向量数据库的 Go 语言 SDK。启用 cgo 时使用 cgo，设置 `CGO_ENABLED=0` 时自动切换到运行时 FFI。
...
- **Go** ≥ 1.21
- **C 编译器**（gcc 或 clang），仅在使用 cgo 时需要
...
后端选择是自动的：`CGO_ENABLED=1` 使用 cgo，`CGO_ENABLED=0` 无需构建标签即可使用运行时 FFI；仍可通过 `-tags purego` 显式强制使用 FFI。两种后端运行时都需要 zvec C-API 动态库；如果动态库不在 SDK 默认目录或可执行文件相对目录中，请设置 `ZVEC_LIBRARY_PATH`。
...
# 3a. 使用 cgo 构建（启用 cgo 时的默认方式）
CGO_ENABLED=1 go build .

# 3b. 或使用运行时 FFI 构建（不需要 C 编译器）
